...
<main class="px-10 flex-1 mt-6">
    <section aria-labelledby="terms" class="flex flex-col gap-8">
        <h2 id="terms"
            class="font-display font-bold text-blue text-5xl tracking-wider uppercase">{{__('Mentions légales')}}</h2>
        <div class="flex flex-col gap-12">
            <div class="flex flex-col gap-2">
                <p class="text-xl text-blue font-semibold">{{__('Qui sommes nous ?')}}</p>
                <p class="">{{__('L’adresse de notre site Web est : ')}}
                    <a href="https://webdesign.be" class="underline underline-offset-2 decoration-1 decoration-solid hover:text-orange">https://webdesign.be</a> </p>
            </div>
            <div class="flex flex-col gap-2">
                <p class="text-xl text-blue font-semibold">{{__('Clause de confidentialité')}}</p>
                <p class="">{{__('WEB DESIGN traitera avec le plus grand soin les données personnelles qui lui ont été communiquées. WEB DESIGN les traitera en toute confidentialité et ne les utilisera qu’aux fins pour lesquelles elles lui ont été communiquées. Ces données peuvent être enregistrées dans une ou plusieurs bases de données de WEB DESIGN et ne seront pas communiquées à des tiers sans le consentement de la personne concernée. Cette dernière peut en outre toujours réclamer la communication de ses données en notre possession, et en demander rectification et suppression.')}}</p>
            </div>
            <div class="flex flex-col gap-2">
                <p class="text-xl text-blue font-semibold">{{__('Contenu embarqué depuis d’autres sites')}}</p>
                <p class="">{{__('Les articles de ce site peuvent inclure des contenus intégrés (par exemple des vidéos, images, articles…). Le contenu intégré depuis d’autres sites se comporte de la même manière que si le visiteur se rendait sur cet autre site. Ces sites web pourraient collecter des données sur vous, utiliser des cookies, embarquer des outils de suivis tiers, suivre vos interactions avec ces contenus embarqués si vous disposez d’un compte connecté sur leur site web.')}}</p>
            </div>
            <div class="flex flex-col gap-2">
                <p class="text-xl text-blue font-semibold">{{__('Utilisation des cookies')}}</p>
