        ...
        <section aria-labelledby="forum" class="flex flex-col gap-8 lg:col-span-3">
            ...
            @auth()
                <div class="flex flex-col gap-6">
                    <form action="/{{app()->getLocale()}}/forum/questions/{{$question->slug}}/edit" method="post"
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <textarea name="body" id="body" cols="30" rows="10"
                                      class="@error('body') error-outline @enderror pl-3 py-2 border border-orange-light rounded-lg focus:outline focus:outline-1 focus:outline-orange placeholder:font-light transition ease-in-out duration-200">{{old('body') ?? $question->body}}</textarea>
                        </div>
                        <div class="flex gap-2">
                            ...
                            <select name="category_id" id="category_id" class="rounded-lg px-4">
                                @foreach($categories as $category)
                                    <option
                                        value="{{$category->category_id}}" @selected($category->category_id === $question->category_id)>{{$category->name}}</option>
                                @endforeach
                            </select>
                        </div>
