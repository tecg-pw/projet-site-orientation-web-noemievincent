    ...
    <div class="flex flex-col gap-3">
        <div>
            <div class="flex justify-between mb-2">
                ...
                @auth()
                    <form action="/{{app()->getLocale()}}/tutorials/save" method="post" class="group">
                        @csrf
                        <input type="hidden" name="user_id" value="{{auth()->user()->id}}">
                        <input type="hidden" name="tuto_id" value="{{$tutorial->tutorial_id}}">
                        <button type="submit" class="flex">
                            <span class="sr-only">{{$is_favorite === 'true' ? __('resources.tutorials.unsave') : __('resources.tutorials.save')}}</span>
                            <span>
